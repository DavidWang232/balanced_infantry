...
/**
 * @brief     角度在极坐标系中的转换 (强制域定到 0~2PI)
 * @param[in] angle  (rad)当前角度
 * @param[in] dangle (rad)旋转角度
 * @param[in] direction 方向，1为正，-1为负
 * @param[in] duration 周期（1个周期0~2*PI, 2个周期0~4*PI, ...）
 * @return    转换后的角度 (0~2PI)
 */
fp32 theta_transform_0_2PI(fp32 angle, fp32 dangle, int8_t direction, uint8_t duration)
{
    return loop_fp32_constrain((angle* direction + dangle), 0, 2.0f * PI * duration);
}
