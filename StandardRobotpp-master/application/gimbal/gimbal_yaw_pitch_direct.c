...
void Angle_solution(void)
{
    float motor_feedback = gimbal_direct.pitch.fdb.pos,
          imu_feedback = gimbal_direct.feedback_pos.pitch, motor_mid = GIMBAL_DIRECT_PITCH_MID,
          imu_mid = 0.0;
...
{
    if (((gimbal_direct.reference.yaw - gimbal_direct.yaw.fdb.pos < 0.003f &&
 ...
 * @retval         bool 解释是否需要继续初始化
 */
inline bool GetGimbalInitJudgeReturn(void) { return gimbal_direct.init_continue; }
...
{
    if (axis == AX_PITCH) {
        return loop_fp32_constrain(
            gimbal_direct.pitch.direction * (gimbal_direct.pitch.fdb.pos - GIMBAL_DIRECT_PITCH_MID),
            -M_PI, M_PI);
    } else if (axis == AX_YAW) {
        return loop_fp32_constrain(
            gimbal_direct.yaw.direction * (gimbal_direct.yaw.fdb.pos - GIMBAL_DIRECT_YAW_MID),
            -M_PI, M_PI);
    } else {
        return 0.0;
    }
...
void GimbalInit(void)
{
    ...
    const static fp32 gimbal_yaw_angle[3] = {
        KP_GIMBAL_YAW_ANGLE, KI_GIMBAL_YAW_ANGLE, KD_GIMBAL_YAW_ANGLE};
    const static fp32 gimbal_yaw_velocity[3] = {
        KP_GIMBAL_YAW_VELOCITY, KI_GIMBAL_YAW_VELOCITY, KD_GIMBAL_YAW_VELOCITY};

    const static fp32 gimbal_pitch_angle[3] = {
        KP_GIMBAL_PITCH_ANGLE, KI_GIMBAL_PITCH_ANGLE, KD_GIMBAL_PITCH_ANGLE};
    const static fp32 gimbal_pitch_velocity[3] = {
        KP_GIMBAL_PITCH_VELOCITY, KI_GIMBAL_PITCH_VELOCITY, KD_GIMBAL_PITCH_VELOCITY};

    PID_init(
        &gimbal_direct_pid.yaw_angle, PID_POSITION, gimbal_yaw_angle, MAX_OUT_GIMBAL_YAW_ANGLE,
        MAX_IOUT_GIMBAL_YAW_ANGLE);
    PID_init(
        &gimbal_direct_pid.yaw_velocity, PID_POSITION, gimbal_yaw_velocity,
        MAX_OUT_GIMBAL_YAW_VELOCITY, MAX_IOUT_GIMBAL_YAW_VELOCITY);

    PID_init(
        &gimbal_direct_pid.pitch_angle, PID_POSITION, gimbal_pitch_angle,
        MAX_OUT_GIMBAL_PITCH_ANGLE, MAX_IOUT_GIMBAL_PITCH_ANGLE);
    PID_init(
        &gimbal_direct_pid.pitch_velocity, PID_POSITION, gimbal_pitch_velocity,
        MAX_OUT_GIMBAL_PITCH_VELOCITY, MAX_IOUT_GIMBAL_PITCH_VELOCITY);

    //step4 初始化电机
    MotorInit(
        &gimbal_direct.yaw, GIMBAL_DIRECT_YAW_ID, GIMBAL_DIRECT_YAW_CAN,
        GIMBAL_DIRECT_YAW_MOTOR_TYPE, GIMBAL_DIRECT_YAW_DIRECTION,
        GIMBAL_DIRECT_YAW_REDUCTION_RATIO, GIMBAL_DIRECT_YAW_MODE);
    MotorInit(
        &gimbal_direct.pitch, GIMBAL_DIRECT_PITCH_ID, GIMBAL_DIRECT_PITCH_CAN,
        GIMBAL_DIRECT_PITCH_MOTOR_TYPE, GIMBAL_DIRECT_PITCH_DIRECTION,
        GIMBAL_DIRECT_PITCH_REDUCTION_RATIO, GIMBAL_DIRECT_PITCH_MODE);
    ...
    //step6 模式设置初始化 - 修改为直接进入IMU模式，避免无力模式导致云台偏移
    gimbal_direct.mode = GIMBAL_IMU;
    gimbal_direct.last_mode = GIMBAL_IMU;
    gimbal_direct.mode_before_rc_err = GIMBAL_IMU;
    gimbal_direct.angle_zero_for_imu = 0;
...
void GimbalObserver(void)
{
    ...
    // 调试数据输出：打印yaw和pitch的IMU反馈角度
    // ModifyDebugDataPackage(0, gimbal_direct.feedback_pos.yaw, "yaw_imu");
    // ModifyDebugDataPackage(1, gimbal_direct.feedback_pos.pitch, "pitch_imu");
...
void GimbalReference(void)
{
    if (gimbal_direct.mode == GIMBAL_INIT) {
        gimbal_direct.reference.pitch = loop_fp32_constrain(
            gimbal_direct.pitch.direction *
                    (GIMBAL_DIRECT_PITCH_MID - gimbal_direct.pitch.fdb.pos) +
                gimbal_direct.feedback_pos.pitch,
            -M_PI, M_PI);
        gimbal_direct.reference.yaw = loop_fp32_constrain(
            gimbal_direct.yaw.direction * (GIMBAL_DIRECT_YAW_MID - gimbal_direct.yaw.fdb.pos) +
        ...
        if (gimbal_direct.last_mode != GIMBAL_IMU) {
            ...
            gimbal_direct.reference.pitch = fp32_constrain(
                gimbal_direct.reference.pitch + gimbal_direct.rc->mouse.y / MOUSE_SENSITIVITY,
                GIMBAL_LOWER_LIMIT_PITCH, GIMBAL_UPPER_LIMIT_PITCH);  //GetDt7MouseSpeed(AX_YAW)
            gimbal_direct.reference.yaw = loop_fp32_constrain(
                gimbal_direct.reference.yaw - gimbal_direct.rc->mouse.x / MOUSE_SENSITIVITY, -M_PI,
                M_PI);  //GetDt7MouseSpeed(AX_PITCH)
            gimbal_direct.reference.pitch = fp32_constrain(
                gimbal_direct.reference.pitch + fp32_deadline(
                                                    gimbal_direct.rc->rc.ch[1],
                                                    REMOTE_CONTROLLER_MIN_DEADLINE,
                                                    REMOTE_CONTROLLER_MAX_DEADLINE) /
                                                    REMOTE_CONTROLLER_SENSITIVITY,
                GIMBAL_LOWER_LIMIT_PITCH, GIMBAL_UPPER_LIMIT_PITCH);
            gimbal_direct.reference.yaw = loop_fp32_constrain(
                gimbal_direct.reference.yaw - fp32_deadline(
                                                  gimbal_direct.rc->rc.ch[0],
                                                  REMOTE_CONTROLLER_MIN_DEADLINE,
                                                  REMOTE_CONTROLLER_MAX_DEADLINE) /
                                                  REMOTE_CONTROLLER_SENSITIVITY,
                -M_PI, M_PI);
            fp32 angle_diff = gimbal_direct.reference.yaw - gimbal_direct.feedback_pos.yaw;
            if (fabsf(angle_diff) > M_PI) {
                if (angle_diff > M_PI) {
                    gimbal_direct.reference.yaw -= 2 * M_PI;
        ...
        gimbal_direct.reference.pitch = fp32_constrain(
            Gimbal_direct_ecd_to_imu(AX_PITCH, GetScCmdGimbalAngle(AX_PITCH)),
            GIMBAL_LOWER_LIMIT_PITCH, GIMBAL_UPPER_LIMIT_PITCH);
        gimbal_direct.reference.yaw = loop_fp32_constrain(
            Gimbal_direct_ecd_to_imu(AX_YAW, GetScCmdGimbalAngle(AX_YAW)), -M_PI, M_PI);
        fp32 angle_diff = gimbal_direct.reference.yaw - gimbal_direct.feedback_pos.yaw;
        if (fabsf(angle_diff) > M_PI) {
            if (angle_diff > M_PI) {
                gimbal_direct.reference.yaw -= 2 * M_PI;
...
void GimbalConsole(void)
{
    ...
    // } else if (
    //     gimbal_direct.mode == GIMBAL_IMU || gimbal_direct.mode == GIMBAL_GAP ||
    //     gimbal_direct.mode == GIMBAL_AUTO_AIM || GIMBAL_INIT) {
    //     gimbal_direct.pitch.set.vel = PID_calc(
    //         &gimbal_direct_pid.pitch_angle, gimbal_direct.feedback_pos.pitch,
    //         gimbal_direct.reference.pitch);
    //     gimbal_direct.pitch.set.curr =
    //         gimbal_direct.pitch.direction * PID_calc(
    //                                             &gimbal_direct_pid.pitch_velocity,
    //                                             gimbal_direct.feedback_vel.pitch,
    //                                             gimbal_direct.pitch.set.vel);

    //     fp32 delta_yaw = loop_fp32_constrain(
    //         gimbal_direct.reference.yaw - gimbal_direct.feedback_pos.yaw, -M_PI, M_PI);
    ...
    //                                  PID_calc(
    //                                      &gimbal_direct_pid.yaw_velocity,
    //                                      gimbal_direct.feedback_vel.yaw, gimbal_direct.yaw.set.vel);
    // }

    // DM4310 MIT模式赋值
    if (gimbal_direct.pitch.type == DM_4310) {
        //gimbal_direct.pitch.set.pos = gimbal_direct.reference.pitch;
        // gimbal_direct.pitch.set.vel =
        //     gimbal_direct.reference.pitch - gimbal_direct.feedback_pos.pitch;
        // gimbal_direct.pitch.set.vel =0;
        gimbal_direct.pitch.set.pos = 0;
        gimbal_direct.pitch.set.vel = PID_calc(
            &gimbal_direct_pid.pitch_angle, gimbal_direct.feedback_pos.pitch,
            gimbal_direct.reference.pitch);
        gimbal_direct.pitch.set.tor = 0.0f;
    }
    if (gimbal_direct.yaw.type == DM_4310) {
        gimbal_direct.yaw.set.pos = 0;
        gimbal_direct.yaw.set.vel = PID_calc(
            &gimbal_direct_pid.yaw_angle, gimbal_direct.feedback_pos.yaw,
...
void GimbalSendCmd(void)
{
    if (gimbal_direct.pitch.type == DM_4310) {
        // 只在pitch轴失能时才发送使能命令
        if (gimbal_direct.pitch.fdb.state == DM_STATE_DISABLE) {
            DmEnable(&gimbal_direct.pitch);
        }
        if (gimbal_direct.yaw.fdb.state == DM_STATE_DISABLE) {
            DmEnable(&gimbal_direct.yaw);
        }
        // MIT模式控制，kp/kd用宏
        DmMitCtrl(&gimbal_direct.pitch, GIMBAL_DM4310_MIT_KP, GIMBAL_DM4310_MIT_KD);
        //yaw轴更换成6006 用DM协议
        DmMitCtrl(&gimbal_direct.yaw, GIMBAL_DM6006_MIT_KP, GIMBAL_DM6006_MIT_KD);
    } else {
        // // pitch轴为大疆电机
        // CanCmdDjiMotor(1, 0x1FF, gimbal_direct.yaw.set.curr, 0, 0, 0);
        // CanCmdDjiMotor(2, 0x1FF, gimbal_direct.pitch.set.curr, 0, 0, 0);
    }
}
