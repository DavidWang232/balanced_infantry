...
#define DM_IMU_CAN_ID 0x09   // DM IMU的CAN ID
#define DM_IMU_STD_ID 0x6FF  // DM IMU请求帧StdId

/**
 * @brief          发送DM IMU数据请求帧 (CAN应答模式)
 * @param[in]      reg 请求的寄存器地址
 * @note           向CAN2的0x6FF发送请求，IMU将以MSTID 0x41回复数据
 * @retval         none
 */
void DM_IMUsend(uint8_t reg)
{
    CanCtrlData_s can_ctrl_data = {
        .hcan = &hcan1,
        .tx_header.StdId = DM_IMU_STD_ID,
        .tx_header.IDE = CAN_ID_STD,
        .tx_header.RTR = CAN_RTR_DATA,
        .tx_header.DLC = 4,
        .tx_data =
            {
                (uint8_t)(DM_IMU_CAN_ID),
                (uint8_t)(DM_IMU_CAN_ID >> 8),
                reg,
                0xCC,
            },
    };
