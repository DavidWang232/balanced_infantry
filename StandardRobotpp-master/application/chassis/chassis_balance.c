...
void ChassisInit(void)
{
    ...
    /*-------------------- 初始化底盘电机 --------------------*/
    /*----------------------左-------------------------------*/
    MotorInit(&CHASSIS.joint_motor[0], 3, JOINT_CAN, DM_8009, J0_DIRECTION, 1, DM_MODE_MIT);
    MotorInit(&CHASSIS.joint_motor[1], 2, JOINT_CAN, DM_8009, J1_DIRECTION, 1, DM_MODE_MIT);
    /*----------------------右-------------------------------*/
    MotorInit(&CHASSIS.joint_motor[2], 4, JOINT_CAN, DM_8009, J2_DIRECTION, 1, DM_MODE_MIT);
    MotorInit(&CHASSIS.joint_motor[3], 1, JOINT_CAN, DM_8009, J3_DIRECTION, 1, DM_MODE_MIT);

    MotorInit(&CHASSIS.wheel_motor[0], 3, WHEEL_CAN, DJI_M3508, W0_DIRECTION, 1, 0);
    MotorInit(&CHASSIS.wheel_motor[1], 4, WHEEL_CAN, DJI_M3508, W1_DIRECTION, 1, 0);
    ...
    float wheel_stand_up_pid_0[3] = {
        KP_CHASSIS_WHEEL_STAND_UP_0, KI_CHASSIS_WHEEL_STAND_UP_0, KD_CHASSIS_WHEEL_STAND_UP_0};
    float wheel_stand_up_pid_1[3] = {
        KP_CHASSIS_WHEEL_STAND_UP_1, KI_CHASSIS_WHEEL_STAND_UP_1, KD_CHASSIS_WHEEL_STAND_UP_1};
    PID_init(
        &CHASSIS.pid.wheel_stand_up[0], PID_POSITION, wheel_stand_up_pid_0,
        MAX_OUT_CHASSIS_WHEEL_STAND_UP_0, MAX_IOUT_CHASSIS_WHEEL_STAND_UP_0);
    PID_init(
        &CHASSIS.pid.wheel_stand_up[1], PID_POSITION, wheel_stand_up_pid_1,
        MAX_OUT_CHASSIS_WHEEL_STAND_UP_1, MAX_IOUT_CHASSIS_WHEEL_STAND_UP_1);
...
void ChassisHandleException(void)
{
    ...
    if ((CHASSIS.mode == CHASSIS_OFF || CHASSIS.mode == CHASSIS_SAFE) &&
        ...
        PID_clear(&CHASSIS.pid.wheel_stand_up[0]);
        PID_clear(&CHASSIS.pid.wheel_stand_up[1]);
    }
}
...
static void UpdateLegStatus(void)
{
    ...
    //     theta_transform_0_2PI(CHASSIS.joint_motor[0].fdb.pos, J0_ANGLE_OFFSET, J0_DIRECTION, 1);
    CHASSIS.fdb.leg[0].joint.Phi1 = M_PI+CHASSIS.joint_motor[0].fdb.pos;

    
    CHASSIS.fdb.leg[0].joint.Phi4 = M_PI_2+CHASSIS.joint_motor[1].fdb.pos;

    ModifyUsart1DebugDataPackage(0,CHASSIS.joint_motor[0].fdb.pos,"Ph1");
    ModifyUsart1DebugDataPackage(1,CHASSIS.joint_motor[1].fdb.pos,"Ph4");
    /*---------右----------*/
    // CHASSIS.fdb.leg[1].joint.Phi1 =
    //     theta_transform_0_2PI(CHASSIS.joint_motor[2].fdb.pos, J2_ANGLE_OFFSET, J2_DIRECTION, 1);
    // CHASSIS.fdb.leg[1].joint.Phi4 =
    //     theta_transform(CHASSIS.joint_motor[3].fdb.pos, J3_ANGLE_OFFSET, J3_DIRECTION, 1);
    CHASSIS.fdb.leg[1].joint.Phi1 = M_PI-CHASSIS.joint_motor[2].fdb.pos;
    CHASSIS.fdb.leg[1].joint.Phi4 = M_PI_2-CHASSIS.joint_motor[3].fdb.pos;
    ...
    for (i = 0; i < 2; i++) {
        ...
        ModifyUsart1DebugDataPackage(2, CHASSIS.fdb.leg[0].rod.L0, "L0_l");
        ModifyUsart1DebugDataPackage(3, CHASSIS.fdb.leg[1].rod.L0, "L0_r");
        ModifyUsart1DebugDataPackage(4, CHASSIS.fdb.leg[0].rod.Phi0,"Phi0_l");
        ModifyUsart1DebugDataPackage(5, CHASSIS.fdb.leg[1].rod.Phi0,"Phi0_r");
...
static void ConsoleStandUp(void)
{
    ...
    // ===驱动轮pid控制（外环：角度→目标速度；内环：速度→电流）===
    float feedforward = 0;
    PID_calc(&CHASSIS.pid.stand_up, CHASSIS.fdb.body.phi, 0);
    float vel_ref = feedforward + CHASSIS.pid.stand_up.out;
    CHASSIS.wheel_motor[0].set.value = PID_calc(
        &CHASSIS.pid.wheel_stand_up[0], CHASSIS.wheel_motor[0].fdb.vel, vel_ref * W0_DIRECTION);
    CHASSIS.wheel_motor[1].set.value = PID_calc(
        &CHASSIS.pid.wheel_stand_up[1], CHASSIS.wheel_motor[1].fdb.vel, vel_ref * W1_DIRECTION);
}
...
static void SendWheelMotorCmd(void)
{
    switch (CHASSIS.mode) {
        ...
        case CHASSIS_FREE: {
            CanCmdDjiMotor(
                WHEEL_CAN, 0x200, CHASSIS.wheel_motor[0].set.tor, CHASSIS.wheel_motor[1].set.tor, 0,
                0);
        } break;
        case CHASSIS_STAND_UP: {
            CanCmdDjiMotor(
                WHEEL_CAN, 0x200, 0, 0, CHASSIS.wheel_motor[0].set.value,
                CHASSIS.wheel_motor[1].set.value);
        } break;
        case CHASSIS_CALIBRATE: {
            CanCmdDjiMotor(WHEEL_CAN, 0x200, 0, 0, 0, 0);
        } break;
        case CHASSIS_OFF: {
            CanCmdDjiMotor(WHEEL_CAN, 0x200, 0, 0, 0, 0);
        } break;
        ...
        default: {
            CanCmdDjiMotor(
                WHEEL_CAN, 0x200, CHASSIS.wheel_motor[0].set.value,
                CHASSIS.wheel_motor[1].set.value, 0, 0);
        }
    }
}
